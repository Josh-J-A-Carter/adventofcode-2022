...
pub fn func() {
    ...
    for line in operations.lines() {
        ...
        for _ in 0..args[0] {
            // Take the last element on the "from" container
            let curr_container = containers.get_mut(&args[1]).unwrap().pop().unwrap();
            // Put this on the "to" container
            containers.get_mut(&args[2]).unwrap().push(curr_container);
        }
    }
